...
__global__ void vector_square_kernel(float* __restrict__ output, const float* input, uint32_t n) {
...
int main() {
    ...
    HIP_CHECK(hipHostMalloc(h_input, bytes));
    ...
    constexpr uint32_t kBlockSize = 256; // 4 wavefronts
    int blocksPerGrid = div_ceil(N, kBlockSize);

    printf("Launch: %d blocks * %d threads\n", blocksPerGrid, kBlockSize);
